...
################################################################################
module "network" {
  source            = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-network-aws"
...
################################################################################
module "bastion" {
  source                    = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-bastion-aws"
  name_prefix               = var.name_prefix
  resource_tag              = random_string.suffix.result
  global_tags               = local.global_tags
...
module "workload" {
  ...
  source         = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-workload-aws"
...
module "cc_asg" {
  source                    = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-asg-aws"
  name_prefix               = var.name_prefix
  resource_tag              = random_string.suffix.result
  global_tags               = local.global_tags
...
################################################################################
module "cc_iam" {
  source              = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-iam-aws"
  iam_count           = 1
  name_prefix         = var.name_prefix
  resource_tag        = random_string.suffix.result
  global_tags         = local.global_tags
...
################################################################################
module "cc_sg" {
  source       = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-sg-aws"
  sg_count     = 1
  name_prefix  = var.name_prefix
  resource_tag = random_string.suffix.result
  global_tags  = local.global_tags
...
################################################################################
module "gwlb" {
  source                = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-gwlb-aws"
  name_prefix           = var.name_prefix
  resource_tag          = random_string.suffix.result
  global_tags           = local.global_tags
  vpc_id                = module.network.vpc_id
...
module "gwlb_endpoint" {
  source              = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-gwlbendpoint-aws"
  name_prefix         = var.name_prefix
  resource_tag        = random_string.suffix.result
  global_tags         = local.global_tags
  vpc_id              = module.network.vpc_id
...
################################################################################
module "route53" {
  source         = "github.com/locozoko/aws_lab/aws_west_vpc_zscc_gwlb_asg_zpa/modules/terraform-zscc-route53-aws"
  name_prefix    = var.name_prefix
  resource_tag   = random_string.suffix.result
  global_tags    = local.global_tags
  vpc_id         = module.network.vpc_id
